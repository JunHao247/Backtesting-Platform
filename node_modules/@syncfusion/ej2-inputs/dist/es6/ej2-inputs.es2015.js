...
let Slider = class Slider extends Component {
    ...
    renderScale() {
        ...
        const min = this.fractionalToInteger(this.min);
        const max = this.fractionalToInteger(this.max);
        const steps = this.fractionalToInteger(smallStep);
        ...
        for (let i = 0, y = !isNullOrUndefined(this.customValues) && this.customValues.length > 0 ?
            ...
            if (!isNullOrUndefined(this.customValues) && this.customValues.length > 0) {
                ...
                if (this.numberOfDecimals(this.max) === 0 && this.numberOfDecimals(this.min) === 0 &&
                    ...
                    const largestep = this.fractionalToInteger(this.ticks.largeStep);
                    const startValue = this.fractionalToInteger(start);
                    if (orien === 'h') {
...
let FormValidator = FormValidator_1 = class FormValidator extends Base {
    ...
    isValid(name, rule) {
        ...
        const dateFormat = ((rule === 'min' || rule === 'max') && this.rules["" + name].date && typeof (this.rules["" + name].date) === 'string') ? this.rules["" + name].date : null;
        const args = { value: this.inputElement.value, param: param, element: this.inputElement, formElement: this.element, format: dateFormat };
...
FormValidator.checkValidator = {
    ...
    date: (option) => {
        if (!isNullOrUndefined(option.param) && (typeof (option.param) === 'string' && option.param !== '')) {
            const globalize = new Internationalization;
...
let OtpInput = class OtpInput extends Component {
    ...
    updateSeparatorValue() {
        if (this.separator === '') {
            for (let i = 0; i < this.separatorElements.length; i++) {
    ...
    getDefaultValue() {
        const extractedValue = typeof this.value === 'number' ? this.value.toString() : this.value;
    ...
    updateInputValue(previousValue) {
        const stringifiedValue = this.value.toString();
        for (let i = 0; i < this.inputs.length; i++) {
            const isValueChanged = previousValue.toString().charAt(i) !== stringifiedValue.charAt(i);
            if (isValueChanged) {
    ...
    updateVariantClass() {
        let variantClass = this.stylingMode.toLocaleLowerCase();
